    ...
    <body>
        <?php include 'includes/navigation.php'; ?>
            ...
            <section class="bg-[#2D3748] md:px-30 px-5 pb-20">
                <form action="update_profile.php" method="POST" enctype="multipart/form-data" class="px-0 md:px-100">
                    <div class="mb-4">
                        <label for="username" class="block mb-2 font-bold text-white">Username</label>
                        <input type="text" id="username" name="username" class="glob-input" placeholder="Enter your new username" required>
                    </div>
                    <div class="mb-4">
                        <label for="email" class="block mb-2 font-bold text-white">Email</label>
                        <input type="email" id="email" name="email" class="glob-input" placeholder="Enter your new email" required>
                    </div>
                    <div class="mb-4">
                        <label for="password" class="block mb-2 font-bold text-white">Password</label>
                        <input type="password" id="password" name="password" class="glob-input" placeholder="Enter your new password" required>
                    </div>
                    <div class="mb-4">
                        <label for="confirm_password" class="block mb-2 font-bold text-white">Confirm Password</label>
                        <input type="password" id="confirm_password" name="confirm_password" class="glob-input" placeholder="Confirm your new password" required>
                    </div>
                    <div class="pt-5 text-center">
                        <button type="submit" class="text-white bg-blue-600 glob-btn hover:bg-blue-500">Update Profile</button>
                    </div>
                </form>
            </section>
            <section class="bg-[#2D3748] md:px-30 px-5 py-10 flex justify-center">
                <form action="functions/logout.php" method="POST">
                    <button type="submit" class="text-white bg-red-600 hover:bg-red-500 glob-btn">Logout</button>
                </form>
            </section>
